...
if (empty($customer['email']) || empty($customer['address']['line1'])) {
    http_response_code(400);
    echo json_encode(['error' => 'Missing contact or shipping information.']);
    exit;
}
...
if ($http_code === 200 && isset($session['url'])) {
    ...
    // Contact + shipping address are collected here (not via Stripe), so they're
    // stored directly on the pending order rather than read back from the session.
