...
<head>
  ...
  <title>Contacts — NoBan Admin</title>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    .table-head{display:grid;grid-template-columns:170px 130px 1fr 130px 140px;gap:16px;padding:14px 20px;border-bottom:1px solid #1e1e1e;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#444;}
    .table-row{display:grid;grid-template-columns:170px 130px 1fr 130px 140px;gap:16px;align-items:center;padding:14px 20px;border-bottom:1px solid #111;transition:background .15s;}
    .table-row:last-child{border-bottom:none;}
    .table-row:hover{background:#1a1a1a;}
    .c-id{font-weight:600;font-size:13px;}
    .c-date{font-size:11px;color:#555;margin-top:2px;}
    .c-detail{font-size:12px;color:#888;line-height:1.5;}
  ...
  <main class="main">
    ...
    <div class="table-wrap">
      ...
      <?php else: ?>
        ...
        <div class="table-row">
          <div>
            <div class="c-id"><?= htmlspecialchars($c['name'] ?? '') ?></div>
            <div class="c-date"><?= htmlspecialchars($c['email'] ?? '') ?></div>
            <div class="c-date"><?= date('M j, Y', strtotime($c['date'] ?? 'now')) ?></div>
          </div>
          <div class="c-detail"><?= htmlspecialchars($c['inquiry'] ?? '—') ?></div>
          <div class="c-detail"><?= nl2br(htmlspecialchars(mb_strimwidth($c['message'] ?? '', 0, 160, '…'))) ?></div>
          <div>
            <span style="color:<?= contactStatusColor($c['status'] ?? 'new') ?>;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;"><?= htmlspecialchars($c['status'] ?? 'new') ?></span>
          </div>
          <div class="actions">
