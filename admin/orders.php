...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    .table-head{display:grid;grid-template-columns:160px 1fr 100px 140px 140px;gap:16px;padding:14px 20px;border-bottom:1px solid #1e1e1e;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#444;}
    .table-row{display:grid;grid-template-columns:160px 1fr 100px 140px 140px;gap:16px;align-items:center;padding:14px 20px;border-bottom:1px solid #111;transition:background .15s;}
    ...
    <div class="table-wrap">
      ...
      <?php else: ?>
        ...
        <div class="table-row">
          ...
          <div>
            <span style="color:<?= orderStatusColor($o['status'] ?? 'pending') ?>;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;"><?= htmlspecialchars($o['status'] ?? 'pending') ?></span>
          </div>
          <div class="actions">
            <form method="POST" style="display:flex;gap:8px;align-items:center;">
              ...
              <select name="status" class="status-select" onchange="this.form.submit()">
                <?php foreach ($statuses as $s): ?>
                  <option value="<?= $s ?>" <?= ($o['status'] ?? 'pending') === $s ? 'selected' : '' ?>><?= ucfirst($s) ?></option>
                <?php endforeach; ?>
              </select>
            </form>
